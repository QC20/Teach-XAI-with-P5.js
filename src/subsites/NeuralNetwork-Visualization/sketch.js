...
function init() {
    ...
    var secondLayerConnections = createLayerConnections(connection, opts.baseSize * 0.8, 6);
    
    // Create the third (outer) layer
    secondLayerConnections.forEach(conn => {
        createLayerConnections(conn, opts.baseSize * 0.6, 4);
    });
...
function createLayerConnections(parentConn, size, count) {
    var layerConnections = [];
    for (var i = 0; i < count; i++) {
        var angle = (i / count) * Tau;
        var dist = opts.baseDist * 2;
        var x = parentConn.x + Math.cos(angle) * dist;
        var y = parentConn.y + Math.sin(angle) * dist;
        var z = parentConn.z + (Math.random() - 0.5) * dist;
        
        var conn = new Connection(x, y, z, size);
        connections.push(conn);
        all.push(conn);
        toDevelop.push(conn);
        layerConnections.push(conn);
        
        // Create a connection between parent and this new node
        parentConn.links.push(conn);
        conn.links.push(parentConn);
    }
